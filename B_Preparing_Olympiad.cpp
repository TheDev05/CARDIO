...
void getSub(vi num, vi temp, vvi *data, int index)
{
    if (index >= num.size())
    {
        if (temp.size())
...
void solve()
{
    ...
    getSub(num, temp, &data, 0);

    int count = 0;
    for (int i = 0; i < data.size(); i++)
    {
        int sum = sumof(data[i]);

        int max = maxin(data[i]);
        int min = minin(data[i]);
