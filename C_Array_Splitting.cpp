...
void solve()
{
    ...
    for (int i = 0; i < data.size(); i++)
    {
        ...
        if ((i > 0 && abs(data[i].second - data[i - 1].second) == 1))
        {
            count--;
        }
        else if (data[i].second == n - 1)
            count--;
    ...
    {
        int index = i;
        while (res[index].first != 0)
        {
            sum += res[index].second;
            index++;
        }
