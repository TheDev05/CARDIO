...
void solve()
{
    ...
    {
        int val = num[i].first, sum = 0;
        for (int j = i + 1; j < num.size(); j++)
        {
            if (sum + num[j].first > val)
            {
                int delta = val - sum;
                sum += delta;
                num[j].first -= delta;

                res[num[i].second].pb(mp(num[j].second, delta));
            ...
            {
                sum += num[j].first;
                res[num[i].second].pb(mp(num[j].second, num[j].first));
    ...
    for (auto j : res)
    {
        ...
        {
            for (int k = 0; k < j.second[i].second; k++)
            {
                atom.pb(mp(j.first, j.second[i].first));
            }
        }
    }
    ...
    {
        std::cout << atom[i].first + 1 << " " << atom[i].second + 1 << '\n';
    }
}