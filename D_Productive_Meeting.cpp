...
void solve()
{
    ...
    while (1)
    {
        ...
        int val1 = data.top().first;
        int index1 = data.top().second;
        data.pop();

        int val2 = data.top().first;
        int index2 = data.top().second;
        data.pop();

        // count, index1,index2
        res.pb(mp(val2, mp(index1, index2)));
        total += val2;

        if (val1 != val2)
        {
            data.push(mp((val1 - val2), index1));
        }
    }
    ...
    for (auto j : res)
    {
        ...
        {
            std::cout << j.second.first << " " << j.second.second << '\n';
        }
    }
}