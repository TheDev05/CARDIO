...
void solve()
{
    ...
    int start = num[0], end = num[n - 1];
    for (int i = 0; i < n; i++)
    {
        if (i == 0)
        {
            std::cout << check(num[i], num[i + 1]) << " " << std::max(check(num[i], end), check(num[i], start)) << '\n';
        }
        else if (i == n - 1)
        {
        ...
        {
            std::cout << std::min(check(num[i], num[i + 1]), check(num[i], num[i - 1])) << " " << std::max(check(num[i], end), check(num[i], start)) << '\n';
        }
    }
}
