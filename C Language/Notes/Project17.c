...
again:
        printf("ENTER YOUR USER ID THEN PASSWORD:\n");
        scanf("%d %d", &userID, &userpwd);

        if (userID == loginID && userpwd == loginpwd)
        {
                printf("WELCOME TO PORTAL\n");
        }
        else
        {
                printf("WRONG CREDETINOLS, TRY AGAIN\n");
                count = count + 1;

                if (count == 3)
                {
                        printf("YOU CROSSED YOUR TRAIL LIMIT\n");
                        return (0);
                }
