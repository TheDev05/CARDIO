...
void _pprintt(pair<T, V> p);
template <class T>
void _pprintt(vector<T> v);
template <class T>
void _pprintt(set<T> v);
template <class T, class V>
void _pprintt(map<T, V> v);
template <class T>
void _pprintt(multiset<T> v);
template <class T, class V>
void _pprintt(pair<T, V> p)
{
    cerr << "{";
...
void blackout_yash()
{
        ...
        fora(i, 0, n * m)
    {
        ...
        fora(j, 0, i + 1)
        {
            if (s[j] == '1')
            {
                int index = i - j, temp1 = index % m, temp2 = index / m;
                row.insert(temp1);
                col.insert(temp2);
                if (j == 0)
                {
                    debug(mp(temp1, temp2))
                }
            }
        }
        cout << size(row) + size(col) << " ";
        // debug(row)
        // debug(col)
    }
    cout << nline;
