// Upsolving wrong submissions
// codeforces-upsolving-helper.herokuapp.com
...
void solve()
{
    ...
    if (val1 > val2)
    {
        val2 = val2 * ceil((double)val1 / val2);
    }

    std::cout << (int)ceil((double)std::max(val1, val2) / std::min(val1, val2)) << '\n';
}