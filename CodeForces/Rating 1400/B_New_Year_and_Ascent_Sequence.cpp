...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < val; j++)
        {
            ...
            if (temp < inox)
            {
                ...
                temp = std::min(temp, inox);

            min = std::min(min, inox);
    ...
    for (auto i : num)
    {
        ...
        {
            res.insert(i.second.second);
            atom[i.second.second]++;
        }
    }
    ...
    for (int i = 0; i < num.size(); i++)
    {
        ...
        else
        {
            ...
            sum += count;
            temp += res.order_of_key(num[i].second.first);

            if (atom.count(num[i].second.first))
            {
                temp += atom[num[i].second.first];
            }
