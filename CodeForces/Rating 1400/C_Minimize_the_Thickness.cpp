...
int fool(vi num, int sum, int index, int mini)
{
    if (index == num.size())
    {
        return (mini);
    ...
    for (int i = index; i < num.size(); i++)
    {
        ...
        if (temp == sum)
        {
            mini = std::max(mini, (i - index) + 1);
            return (fool(num, sum, i + 1, mini));
        }
    }
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (fool(num, sum, i + 1, i + 1))
        {
            inox = std::min(inox, fool(num, sum, i + 1, i + 1));
        }
    }
