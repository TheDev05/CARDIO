...
int fool(int val, int key, int count)
{
    if ((val == key || inox.count(val) == false) && count != 0)
    {
        return (count);
    }

    return (fool(inox[val], key, count + 1));
}
...
void solve()
{
    ...
    loop(0, n)
    {
        ...
        int min = std::min(num[i].first, num[i].second);
        int max = std::max(num[i].first, num[i].second);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int count = fool(num[i].first, num[i].first, 0);
        // db(count);
