...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (text[i] == 'r')
        {
            ...
            {
                if (index < n && text[index] == 'b')
                {
                    sum += (count / 2);
                    swap(text[index], text[index - 1]);
                }
                else
                    data['b'] += count / 2;
            }
    ...
    std::cout << sum + std::max(data['r'], data['b']) << '\n';
}
