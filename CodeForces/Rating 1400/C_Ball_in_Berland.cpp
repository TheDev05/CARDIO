...
void solve()
{
    ...
    loop(0, n)
    {
        data[mp(num[i].first, num[i].second)]++;
    }

    int count = 0, total = n;
    loop(0, n - 1)
    {
        int val1 = total - first[num[i].first];
        int val2 = second[num[i].second];
        int val3 = data[mp(num[i].first, num[i].second)];
        ...
        if (first[num[i].first] == 0)
            first.erase(num[i].first);
        if (second[num[i].second] == 0)
            second.erase(num[i].second);
        if (data[mp(num[i].first, num[i].second)] == 0)
            data.erase(mp(num[i].first, num[i].second));
