...
void solve()
{
    ...
    int max = maxin(num), min = minin(num);

    for (int i = 0; i < t; i++)
    {
        int defense, attack, sum = 0;
        std::cin >> defense >> attack;

        auto it = data.lower_bound(defense);
        if (it == data.end())
        {
            sum += defense - max;
            int delta = total_sum - max;
