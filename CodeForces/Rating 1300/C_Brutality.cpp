...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        data.pb(mp(text[i], num[i]));
    }

    multiset<int, greater<>> temp;
    vector<pair<char, multiset<int, greater<>>>> res;
    for (int i = 0; i < data.size(); i++)
    {
        int index = i;
        res.pb(mp(text[i], temp));

        while (data[i].first == data[index].first && index < n)
        {
            res[res.size() - 1].second.insert(data[index].second);
            index++;
        }
    ...
    {
        auto it = res[i].second.begin();
        for (int j = 0; (j < k) && (j < res[i].second.size()); j++)
        {
            sum += (*it);
            it++;
        }
