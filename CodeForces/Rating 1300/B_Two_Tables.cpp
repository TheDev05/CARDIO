...
void solve()
{
    ...
    if (abs(n - delta1) < p && abs(m - delta2) < q)
    {
        std::cout << "-1\n";
        rn;
    }

    if (abs(a - 0) >= p || abs(n - x) >= p)
    {
        std::cout << "0.000000000\n";
        rn;
    }
    else if (abs(b - 0) >= q || abs(y - m) >= q)
    {
        std::cout << "0.000000000\n";
        rn;
    }

    int val1 = imax, val2 = imax, val3 = imax, val4 = imax;
    if (abs(n - delta1) >= p)
    {
        val1 = abs(abs(a - 0) - p);
        val2 = abs(abs(x - n) - p);
    ...
    std::cout << (double)std::min({val1, val2, val3, val4}) << '\n';
}
