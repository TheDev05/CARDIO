...
void solve()
{
    ...
    vector<pair<pair<int, int>, int>> num(r);
    for (int i = 0; i < r; i++)
    {
        int val1, val2, val3;
        std::cin >> val1 >> val2 >> val3;

        num[i].first.first = val1;
        num[i].first.second = val2;
        num[i].second = val3;
    }
    ...
    {
        if (data.count(num[i].first.first) && data.count(num[i].first.second))
        {
            continue;
        }
