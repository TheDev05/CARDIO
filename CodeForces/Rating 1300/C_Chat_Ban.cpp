...
void solve()
{
    ...
    ll first_sum = (n * (n + 1)) / 2;
    ll temp = n - 1;
    ll last_sum = (temp * (temp + 1)) / 2;

    if (last_sum + first_sum <= k)
    {
        std::cout << (2 * n) - 1 << '\n';
        rn;
    }
    ...
    else
    {
        ll val = n;
        ll a = n - 1;
        ll delta = k - first_sum;

        double d = sqrt((((double)(2 * a) + 1) * ((2 * a) + 1)) - (4 * (2 * delta)));
        val += ceil((((double)(2 * a) + 1) - d) / 2);
