...
void solve()
{
    ...
    ll max = imin;
    ll left = imax, right = imin;

    bool ok = false;

    if (index >= num[0].first && index <= num[0].second)
    {
        max = std::max(max, (num[0].second - num[0].first) + 1);
    ...
    for (ll i = 1; i < m; i++)
    {
        ll l = num[i].first, r = num[i].second;
        ...
        if (ok)
        {
            ...
            else if ((l < left && l < right) && r > left && r > right)
            {
