...
void solve()
{
    ...
    {
        double inox = data[i].second, size = data[i].first.size();
        max = std::max(max, inox / size);

        // db(max);

        for (int j = 0; (data[i].first.size() - j) > k; j++)
        {
            inox = inox - data[i].first[j];
            max = std::max(max, (inox / (size - (j + 1))));
        }
    }
