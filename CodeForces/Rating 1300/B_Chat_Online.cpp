...
void solve()
{
    ...
    int n, m, left, right;
    std::cin >> n >> m >> left >> right;

    vpi num1(n), num2(m), result;
    set<int> data;
    ...
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            int val1 = num1[j].second - num2[i].first;
            int val2 = num1[j].first - num2[i].second;
            ...
            val2 = std::max(temp, val2);

            if (val1 > right || val2 < left)
            {
                continue;
            }
