...
void solve()
{
    ...
    for (int i = 0; i < res.size(); i++)
    {
        ...
        {
            int index = i;
            while ((index < res.size() - 1) && (res[index + 1].first - res[index].first == 1) && (res[index].second >= m) && (res[index + 1].second >= m))
            {
                index++;
            }

            result.pb(mp(res[i].first, res[index].first));
            i = index;
        }
    }
    ...
    {
        int delta = result[i].second - result[i].first;
        if (delta > max)
        {
            max = delta;
            val1 = result[i].first;
