...
void solve()
{
    ...
    double val1 = (((double)2 * b) - c) / a;
    double val2 = (((double)2 * b) - a) / c;
    double val3 = (((double)a + c) / 2) / b;

    // db(val1);
    // db(val2);
    // db(val3);

    if (ceil(val1) == floor(val1) && val1 > 0 || ceil(val2) == floor(val2) && val2 > 0 || ceil(val3) == floor(val3) && val3 > 0)
    {
        std::cout << "YES\n";
    }
    else
