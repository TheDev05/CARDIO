...
int read(vi num1, vi num2)
{
    int sum1 = 0;
    for (int i = 0; i < num1.size(); i++)
    {
        if (find(all(num2), num1[i] - 1) != num2.end())
        {
            auto it = find(all(num2), num1[i] - 1);
            sum1++;

            *(it) = -1;
        }
        else if (find(all(num2), num1[i]) != num2.end())
        {
            auto it = find(all(num2), num1[i]);
            sum1++;

            *(it) = -1;
        }
        else if (find(all(num2), num1[i] + 1) != num2.end())
        {
            auto it = find(all(num2), num1[i] + 1);
            sum1++;
...
void solve()
{
    ...
    int sum1 = read(num1, num2);
    int sum2 = read(res, num1);
