...
void fool(vi num, int index, int total)
{
    if (index == num.size() - 1)
    {
        db(total);
    ...
    for (int i = index; i < num.size() - 1; i++)
    {
        if (dp[i] == -1 && dp[i - 1] == -1)
        {
            if (std::max(num[i - 1], num[i + 1]) >= num[i])
            {
                sum = ((std::max(num[i - 1], num[i + 1])) + 1) - num[i];
            }

            dp[i] = 0;
            total += sum;
            fool(num, index + 1, total);
