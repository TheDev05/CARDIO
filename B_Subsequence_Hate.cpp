...
void solve()
{
    ...
    int zeroLeft, zeroRight, oneLeft, oneRight;
    ...
    for (int i = zeroLeft; i <= zeroRight; i++)
    {
        if (num[i].first == '1')
        {
            sumZero += num[i].second;
        }
    }

    if (num[0].first == '1' && num[num.size() - 1].first == '1')
    {
        sumZero += std::min(num[0].second, num[num.size() - 1].second);
    }

    for (int i = oneLeft; i <= oneRight; i++)
    {
        if (num[i].first == '0')
        {
            sumOne += num[i].second;
    ...
    if (num[0].first == '0' && num[num.size() - 1].first == '0')
    {
        sumOne += std::min(num[0].second, num[num.size() - 1].second);
    }

    std::cout << std::min(sumOne, sumZero) << '\n';
}