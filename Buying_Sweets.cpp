...
void solve()
{
    ...
    vector<pair<ll, pair<ll, ll>>> num(n);
    for (ll i = 0; i < n; i++)
    {
        ll val;
        std::cin >> val;

        num[i].second.first = val;
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        num[i].second.second = val;
    }

    for (ll i = 0; i < n; i++)
    {
        ll delta = num[i].second.first - num[i].second.second;
        num[i].first = delta;
    }
    ...
    {
        if (num[i].second.first <= k)
        {
            ll window = (k - num[i].second.first) + 1;
            ll val = ceil((double)window / num[i].first);
