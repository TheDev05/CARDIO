...
void solve()
{
	...
	{
		int val = num[i].first;
		int quot = num[i].second / num[i].first;
		int rem = num[i].second % num[i].first;
	...
	if (n == 1)
	{
		std::cout << count + (k / data[0].second.first) << '\n';
		rn;
	}
	...
	for (int i = 0; i < data.size() - 1; i++)
	{
		...
		int delta = data[i + 1].first - data[i].first;
		debt += data[i].second.second;
	...
	k += data[data.size() - 1].second.second;
	sum += data[data.size() - 1].second.first;
	count += k / sum;
