...
int fool(std::string text, std::string res)
{
    int ans = 0;
    for (int i = 0; i < res.size(); i++)
    {
        if (res[i] == text[0])
        {
            ...
            while (index1 < res.size() && index2 < text.size())
            {
                if (res[index1] == text[index2])
                {
...
void solve()
{
    ...
    std::string text = "abacaba", res, inox = "No";
    std::cin >> res;

    for (int i = 0; i < res.size(); i++)
    {
        if (res[i] == text[0] || res[i] == '?')
        {
            int index1 = i, index2 = 0, count = 0;
            ...
            while (index1 < res.size() && index2 < text.size())
            {
                if (res[index1] == text[index2] || res[index1] == '?')
                {
                    temp[index1] = text[index2];
                    count++;
                }
