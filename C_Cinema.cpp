...
void solve()
{
    ...
    vpi res(m);
    loop(0, m) std::cin >> res[i].first;
    loop(0, m) std::cin >> res[i].second;

    bool ok = true;

    int index, max1 = imin, max2 = imin;
    for (int i = 0; i < m; i++)
    {
        int val1 = res[i].first, val2 = res[i].second;
        if (data.count(val1))
        {
            ok = false;
            if (data[val1] > max1)
            ...
            else if ((data[val1] == max1) && (data[val2] > max2))
            {
                index = i + 1;
                max2 = data[val2];
            }
    ...
    if (ok)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            if (data[res[i].second] > max1)
            {
                index = i + 1;
                max1 = data[res[i].second];
            }
        }
    }
