...
void solve()
{
    ...
    int coin = 0, attempts = 0, clears = 0;
    for (int i = 0; i < n; i++)
    {
        if (num[i].first < attempts || num[i].second < clears)
        {
            std::cout << "NO\n";
            rn;
        }

        int delta_attempt = num[i].first - attempts;
        int delta_clears = num[i].second - clears;

        if (delta_clears > delta_attempt)
        {
            std::cout << "NO\n";
            rn;
        }

        attempts = num[i].first;
        clears = num[i].second;
    }
